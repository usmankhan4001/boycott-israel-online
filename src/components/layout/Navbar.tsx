...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="bg-emerald-600 dark:bg-emerald-700 text-white rounded-b-3xl p-4 sm:p-5 shadow-sm space-y-3.5 relative z-30 transition-all sticky top-0 backdrop-blur-md">
      <div className="flex items-center justify-between gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-1 bg-black/15 p-1 rounded-2xl text-xs font-bold text-emerald-50 backdrop-blur-md">
          ...
          <Link 
            to="/about" 
            className={`px-3 py-1.5 rounded-xl transition-all ${
              location.pathname === '/about' ? 'bg-white text-emerald-950 shadow-xs font-black' : 'hover:bg-white/10 text-white'
            }`}
          >
            {t.whyBoycott}
          </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          >
            ...
            <span className="hidden md:inline">{isUrdu ? 'تلاش کریں' : 'Search'}</span>
          ...
          <Link 
            to="/suggest" 
            className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-bold transition-all active:scale-95 border border-white/10"
          >
      ...
      {isMenuOpen && (
        <nav className="lg:hidden flex flex-col gap-1.5 pt-2 pb-1 border-t border-white/15 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          >
            ...
            <span className="px-2 py-0.5 rounded-md bg-amber-400/20 text-amber-200 text-[10px] font-black">
              {isUrdu ? 'فورم' : 'Live'}
            </span>
          </Link>
          <Link 
          ...
          <Link 
            ...
            onClick={() => setIsMenuOpen(false)} 
            className={`px-3.5 py-2 rounded-xl font-bold text-xs flex items-center justify-between ${
              location.pathname === '/about' ? 'bg-white text-emerald-900 shadow-xs font-black' : 'bg-white/10 text-white'
            }`}
          >
            <span>{t.whyBoycott}</span>
          </Link>
          <Link 
            to="/suggest" 
            onClick={() => setIsMenuOpen(false)} 
            className="px-3.5 py-2 rounded-xl font-bold text-xs bg-white/10 text-white flex items-center gap-2"
          >
            <PlusCircle className="w-3.5 h-3.5" />
            <span>{t.suggest}</span>
          </Link>
      ...
      {/* Embedded Search Bar (on Home Page) with quick Scanner link */}
      {location.pathname === '/' && (
        <div className="pt-1">
          <div className="relative flex items-center bg-white dark:bg-zinc-900 rounded-2xl p-1.5 shadow-md border border-white/20 dark:border-zinc-700 transition-all">
            ...
            {searchQuery ? (
              ...
              <button
                onClick={() => setCommandPaletteOpen(true)}
                className="hidden sm:inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-400 text-[10px] font-mono mr-2"
