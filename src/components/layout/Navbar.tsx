...
import { Sun, Moon, Search, ScanLine, ShoppingCart, Download, Menu, X, PlusCircle, Globe } from 'lucide-react';
    ...
    <header className="bg-emerald-600 dark:bg-emerald-700 text-white rounded-b-2xl p-4 sm:p-6 shadow-sm space-y-4 relative z-30 transition-all sticky top-0">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-black/15 p-1 rounded-xl text-xs font-bold text-emerald-50">
          <Link to="/" className={`px-3 py-1.5 rounded-lg transition-all ${location.pathname === '/' ? 'bg-white text-emerald-900 shadow-xs font-black' : 'hover:bg-white/10 text-white'}`}>{t.allBrands}</Link>
          <Link to="/categories" className={`px-3 py-1.5 rounded-lg transition-all ${location.pathname === '/categories' ? 'bg-white text-emerald-900 shadow-xs font-black' : 'hover:bg-white/10 text-white'}`}>{t.categories}</Link>
          <Link to="/grocery" className={`px-3 py-1.5 rounded-lg transition-all ${location.pathname === '/grocery' ? 'bg-white text-emerald-900 shadow-xs font-black' : 'hover:bg-white/10 text-white'}`}>{t.grocery} ({groceryList.length})</Link>
          <Link to="/about" className={`px-3 py-1.5 rounded-lg transition-all ${location.pathname === '/about' ? 'bg-white text-emerald-900 shadow-xs font-black' : 'hover:bg-white/10 text-white'}`}>{t.whyBoycott}</Link>
        </nav>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={toggleTheme} className="p-2 rounded-xl bg-white/15 hover:bg-white/25 text-white backdrop-blur-md transition-colors" title="Theme">
      ...
      {isMenuOpen && (
        <nav className="md:hidden flex flex-col gap-2 mt-4 animate-in fade-in">
          <Link to="/" onClick={() => setIsMenuOpen(false)} className="px-3 py-2 bg-white/10 rounded-lg text-white font-semibold">{t.allBrands}</Link>
          <Link to="/categories" onClick={() => setIsMenuOpen(false)} className="px-3 py-2 bg-white/10 rounded-lg text-white font-semibold">{t.categories}</Link>
          <Link to="/grocery" onClick={() => setIsMenuOpen(false)} className="px-3 py-2 bg-white/10 rounded-lg text-white font-semibold">{t.grocery}</Link>
          <Link to="/about" onClick={() => setIsMenuOpen(false)} className="px-3 py-2 bg-white/10 rounded-lg text-white font-semibold">{t.whyBoycott}</Link>
        </nav>
      )}
      ...
      {location.pathname === '/' && (
        <div className="pt-2">
          <div className="relative flex items-center bg-white dark:bg-zinc-900 rounded-xl p-1.5 shadow-md border border-white/20 dark:border-zinc-700 transition-all">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 mx-1">
                <X className="w-4 h-4" />
              </button>
            )}
