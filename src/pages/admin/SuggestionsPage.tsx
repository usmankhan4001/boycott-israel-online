  ...
  return (
    <div className="space-y-5 sm:space-y-6 max-w-5xl min-w-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white">Community Submissions</h2>
        ...
        <button
          ...
          className="self-start sm:self-auto flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 text-xs font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors shadow-2xs"
        >
      ...
      {loading ? (
        ...
        <div className="p-8 sm:p-12 text-center rounded-2xl sm:rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
          {suggestions.map(s => (
            <div key={s.id} className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 space-y-3.5 shadow-xs flex flex-col justify-between min-w-0">
              <div className="space-y-3 min-w-0">
                <div className="flex justify-between items-start gap-2">
                  <div className="min-w-0">
                    <h3 className="font-black text-base text-zinc-900 dark:text-white truncate">{s.brandName}</h3>
                    <p className="text-[11px] text-zinc-500 font-semibold truncate">{s.category || 'General'}</p>
                  </div>
                  <span className="px-2 py-0.5 text-[10px] font-black rounded-md bg-rose-100 text-rose-800 dark:bg-rose-950/60 dark:text-rose-300 uppercase shrink-0">
                    {s.type || 'BOYCOTT'}
                  </span>
                </div>
                
                <div className="bg-zinc-50 dark:bg-zinc-800/60 p-3 rounded-xl sm:rounded-2xl space-y-1.5 text-xs text-zinc-700 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-800 break-words">
