export async function onRequestPost(context: any) {
  ...
  try {
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_geopolitical_zones_name ON geopolitical_zones(zone_name)`).run();

    // 7. Raw resources table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_raw_resources_zone ON raw_resources(primary_extraction_zone_id)`).run();

    // 8. Parent conglomerates table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_parent_conglomerates_name ON parent_conglomerates(entity_name)`).run();

    // 9. Complicity edges table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_complicity_edges_conglomerate ON complicity_edges(conglomerate_id)`).run();

    // 10. Retail brands table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_retail_brands_type ON retail_brands(brand_type)`).run();

    // 11. Alternative profiles table
    await env.DB.prepare(`
      CREATE TABLE IF NOT EXISTS alternative_profiles (
        ...
        editorial_badge TEXT DEFAULT 'fully_recommended',
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_alternative_profiles_brand ON alternative_profiles(brand_id)`).run();

    // 12. Market gaps table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_market_gaps_category ON market_gaps(category)`).run();

    // 13. Toxic additives table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_toxic_additives_code ON toxic_additives(code)`).run();

    // 14. User impact ledger table
    await env.DB.prepare(`
    ...
    `).run();
    await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_user_impact_user ON user_impact_ledger(user_identifier)`).run();
...
export async function onRequestGet(context: any) {
  ...
  const counts: Record<string, number> = {
    ...
    geopoliticalZones: 0,
    rawResources: 0,
    parentConglomerates: 0,
    complicityEdges: 0,
    retailBrands: 0,
    alternativeProfiles: 0,
    marketGaps: 0,
    toxicAdditives: 0,
    userImpactLedger: 0
  };
  ...
  if (isBound) {
    const tableQueries: [string, string][] = [
      ...
      ['geopoliticalZones', 'geopolitical_zones'],
      ['rawResources', 'raw_resources'],
      ['parentConglomerates', 'parent_conglomerates'],
      ['complicityEdges', 'complicity_edges'],
      ['retailBrands', 'retail_brands'],
      ['alternativeProfiles', 'alternative_profiles'],
      ['marketGaps', 'market_gaps'],
      ['toxicAdditives', 'toxic_additives'],
      ['userImpactLedger', 'user_impact_ledger']
    ];

    for (const [key, tbl] of tableQueries) {
      try {
        const res = await env.DB.prepare(`SELECT COUNT(*) as c FROM ${tbl}`).first();
        counts[key] = res?.c || 0;
      } catch {}
    }
  }
  ...
  return new Response(JSON.stringify({
    ...
    postCount: counts.communityPosts,
    commentCount: counts.postComments,
    notificationCount: counts.appNotifications
  }), {
    status: 200,
    headers: { 'Content-Type': 'application/json' }
  });
